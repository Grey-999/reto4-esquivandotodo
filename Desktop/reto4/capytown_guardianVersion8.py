...
class CapyGuardian(Node):
    ...
    def _points_robot_frame(self, msg: LaserScan) \
        ...
        +x = adelante, +y = izquierda.  Aplica offset angular del LiDAR,
        signo del eje Y, y filtros de rango / zona angular ignorada.
        """
        ...
        for i, r in enumerate(msg.ranges):
            ...
            theta = self.lidar_y_sign * theta_lidar + self.lidar_offset
            # Normalizar a [-π, π]
            theta = math.atan2(math.sin(theta), math.cos(theta))
            if self._in_ignore_arc(theta):
                continue
            pts.append((i, r * math.cos(theta), r * math.sin(theta)))
        return pts
